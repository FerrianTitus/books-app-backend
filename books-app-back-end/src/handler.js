...
const getAllBooksHandler = (request, h) => {
  ...
  if (reading) {
    ...
    const response = h.response({
      status: 'success',
      data: {
        books: books.filter((book) => Number(book.finished === true) === Number(finished))
          .map((book) => ({
            id: book.id,
            name: book.name,
    ...
    const response = h.response({
      status: 'success',
      data: {
        books: books.filter((book) => Number(book.finished === false) !== Number(finished))
          .map((book) => ({
            id: book.id,
            name: book.name,
            publisher: book.publisher,
...
const getBooksByIdHandler = (request, h) => {
  ...
  const book = books.filter((f) => f.id === bookId)[0];
  if (book !== undefined) {
    const response = h.response({
      status: 'success',
      data: {
